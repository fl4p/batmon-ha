...
def main():
    ...
    if total_rx == 0:
        ...
        print('  1. GND. On the Daly XH 5-pin RS485/CAN cable that is pin 3')
        print('     (1 = B-, 2 = A+, 3 = GND, 4-5 = CAN). Without GND there is no')
        print('     communication at all -- confirmed on a Daly 4S in #398.')
        print('  2. RS485 A/B swapped -- the single most common cause. Swap them.')
        print('  3. Right port on the BMS. Daly UART (TTL, 3.3V) and RS485 are')
        print('     separate ports with different pinouts. A bare USB-TTL cable')
        print('     belongs on the UART port; RS485 needs a transceiver (MAX485).')
        print('     (A USB-RS485 adapter built on an FTDI chip is fine -- the')
        print('     transceiver is on the adapter board, not in the chip name.)')
        print('  4. On SOME boards the UART port shares its UART with the')
        print('     Bluetooth module, so BT and a wired link cannot both run.')
        print('     Others (incl. recent ones with two UART headers) do allow it.')
        print('     If unsure, unplug the BT module and retest.')
