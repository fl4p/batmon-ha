...
* Fix: a lower-case `address:` never connected through an ESPHome proxy. habluetooth looks addresses up as plain dict keys and a proxy always reports them upper-case, so the address missed and the failure surfaced as `No backend with an available connection slot that can reach address …` — which reads like the proxy being out of connection slots, not a spelling mismatch. MAC addresses are now canonicalized to upper case on both the native and the `_ble`/aiobmsble path (the latter failed with `BleakDeviceNotFoundError` instead, same cause). BlueZ accepts either case, so only `ble_stack: esphome` was affected (#399).
* With `ble_stack: esphome`, start-up no longer prints `BluetoothManager: does not implement _discover_service_info, subclasses must implement this method to consume discovery data`. The hook is optional — batmon polls a configured address rather than reacting to discoveries — but the line read like a fault (#399).
* Fix: canonicalizing the address (above) broke a `group_parallel` whose members were referenced by lower-case MAC — the group could no longer find them and the add-on aborted at start-up instead of just the group failing. Member references now match a name exactly or an address case-insensitively (#399).
* Anonymous telemetry keeps hashing the address as it is written in the config, not the canonicalized form, so a device's telemetry identity does not move at this upgrade (#399).
